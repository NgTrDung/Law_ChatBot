import os
import module_RAG.gemini_Generate_Queries as g_G_Q
import re
...
def search_documents_with_should_filter(user_query, metadata_fields = metadata_fields, top_k=5):
...
def combined_search(user_query: str, key_manager, metadata_fields = metadata_fields, top_k=5):
...
def handle_Query(user_Query, key_manager):
    search_Article_Section_Results = combined_search(user_Query, key_manager, metadata_fields, top_k=5)

    re_Rank_Article_Section_Results = calculate_scores(user_Query, search_Article_Section_Results)
